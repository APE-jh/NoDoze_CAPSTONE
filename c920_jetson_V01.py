...
# GPIO 설정 (Jetson Nano, Raspberry Pi)
GPIO.setmode(GPIO.BOARD)
motor_pin = 18  # 사용할 GPIO 핀 번호 (18번 핀)
GPIO.setup(motor_pin, GPIO.OUT)

# 카메라 연결 (Logitech C920, 인덱스 1)
cap = cv2.VideoCapture(1)  # 웹캠 인덱스 1로 설정 (기본 내장 카메라는 인덱스 0)
...
def calculate_head_tilt(landmarks):
    ...
    dx = nose.x - chin.x
    dy = nose.y - chin.y
    angle = math.atan2(dy, dx) * (180.0 / math.pi)  # math 모듈을 사용하여 각도 계산
    return angle
...
while True:
    ...
    if results.multi_face_landmarks:
        for landmarks in results.multi_face_landmarks:
            ...
            # 졸음운전 감지 시 GPIO 출력
            if drowsy_detected:
                GPIO.output(motor_pin, GPIO.HIGH)  # 모터 작동 (GPIO 핀을 HIGH로 설정)
                time.sleep(1)  # 1초 동안 모터 작동
                GPIO.output(motor_pin, GPIO.LOW)  # 모터 정지
